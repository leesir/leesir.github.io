...
&#160; &#160; &#160; &#160;一般来说，多期的回款或者还款计划会在项目成立（也就是满标），放款完成之后生成。
在后续的所有流程中，均不会更改已生成的金额数据，比如在还款成功之后，只会变更计划的状态为已还清。对于本文中可能会出现的名词，现如下定义：

> 投资人：出钱的人。当借贷关系成立时，资金流出方，当借贷关系结束时，资金流入方。
>
> 借款人：借钱的人。当借贷关系成立时，资金流入方，当借贷关系结束时，资金流出方。
>
> 标的：将借款人的需求包装而成的金融产品。
>
...
***重要说明：[等额本息](https://baike.baidu.com/item/%E7%AD%89%E9%A2%9D%E6%9C%AC%E6%81%AF)的重点在于每月偿还本息一致，或者还款总额一致。不同机构给出的还款计划的利息、本金、剩余本金明细可能不完全一致，
...
&#160; &#160; &#160; &#160;当然在实际情况中，借款肯定是有服务费的（监管规定总费率不能超过36%），此时标准的等额本息公式已经不再适用了。
...
&#160; &#160; &#160; &#160;1. 本文从投资人优先的角度，先计算投资人的计划数据，再由此生成借款人的计划数据。由一份数据源，累加生成后续所需的数据源，从而保证了金额的准确性。
在计算借款人计划时，不能再运用等额本息公式进行计算，否则会因为精度问题导致每期还款总额与出借人应收总额不一致。

&#160; &#160; &#160; &#160;2. 如果需要把某一个数摊为n期，建议在1～n-1期都用向下取整（只舍不入），并在第n期计算差值，这样可以保证1～n期的总和准确，并且第n期不会成为负数。
